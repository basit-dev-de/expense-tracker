export const de = {
  // Expense Tracker translations
  expenseTracker: "Ausgabenverfolger",
  dashboard: "Übersicht",
  transactions: "Transaktionen",
  budgets: "Budgets",
  reports: "Berichte",
  settings: "Einstellungen",
  addExpense: "Ausgabe hinzufügen",
  addIncome: "Einkommen hinzufügen",
  categories: "Kategorien",
  amount: "Betrag",
  date: "Datum",
  description: "Beschreibung",
  category: "Kategorie",
  type: "Typ",
  expense: "Ausgabe",
  income: "Einkommen",
  save: "Speichern",
  cancel: "Abbrechen",
  delete: "Löschen",
  edit: "Bearbeiten",
  total: "Gesamt",
  balance: "Kontostand",
  overview: "Überblick",
  recentTransactions: "Neueste Transaktionen",
  monthlySpending: "Monatliche Ausgaben",
  topCategories: "Top Kategorien",
  budgetStatus: "Budget-Status",
  status: "Status",
  completed: "Abgeschlossen",
  pending: "Ausstehend",
  remaining: "Verbleibend",
  used: "Verwendet",
  of: "von",
  viewAll: "Alle anzeigen",
  welcomeBack: "Willkommen zurück",
  todayDate: "Heute, {{date}}",

  // Common UI elements
  lightTheme: "Hell",
  darkTheme: "Dunkel",
  systemTheme: "System",
  language: "Sprache",
  english: "Englisch",
  german: "Deutsch",

  // Categories
  food: "Essen & Trinken",
  transport: "Transport",
  housing: "Wohnen",
  entertainment: "Unterhaltung",
  shopping: "Einkaufen",
  utilities: "Nebenkosten",
  healthcare: "Gesundheit",
  education: "Bildung",
  personal: "Körperpflege",
  travel: "Reisen",
  other: "Sonstiges",

  // Success messages
  transactionAdded: "Transaktion erfolgreich hinzugefügt",
  transactionUpdated: "Transaktion erfolgreich aktualisiert",
  transactionDeleted: "Transaktion erfolgreich gelöscht",
  budgetCreated: "Budget erfolgreich erstellt",
  budgetUpdated: "Budget erfolgreich aktualisiert",
  settingsSaved: "Einstellungen erfolgreich gespeichert",
  thisMonth: "Diesen Monat",
  yourLatestTransactions: "Deine neuesten Transaktionen",

  // Settings
  appearance: "Erscheinungsbild",
  customizeAppearance: "Dein Erscheinungsbild anpassen",
  theme: "Thema",
  notifications: "Benachrichtigungen",
  manageNotifications: "Deine Benachrichtigungen verwalten",
  emailNotifications: "E-Mail-Benachrichtigungen",
  budgetAlerts: "Budget-Benachrichtigungen",
  weeklySummary: "Wochenbericht",
  privacy: "Datenschutz",
  managePrivacy: "Deine Datenschutz-Einstellungen verwalten",
  saveHistory: "Transaktionsverlauf speichern",
  analytics: "Analytics",
  account: "Konto",
  manageAccount: "Dein Konto verwalten",
  clearAllData: "Alle Daten löschen",
  exportData: "Daten exportieren",
  importData: "Daten importieren",
  saveSettings: "Einstellungen speichern",
};
